...
/*   Updated: 2024/11/23 17:05:18 by iatilla-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_conditions(const char *format, int i, va_list args, int *bytes)
{
	...
	if (format[i] == 's')
	{
		...
		handle_strings(datatype.string, bytes);
		i++;
	}
	else if (format[i] == 'p')
	{
		current = va_arg(args, void *);
		put_pointer(current, bytes);
		i++;
	}
...
int handle_percent(const char *format, int *i, int *bytes)
{
    ...
    // Check if the next character is not a valid specifier
        ...
        !(format[*i + 1] >= '0' && format[*i + 1] <= '9'))
    {
        written = write(1, "%", 1);
        *bytes += written;
        (*i)++;  // Correctly increment the pointer
        j = -1;
    }

    // Check for the case where the next character is a percent sign
    if (format[*i + 1] == '%')
    {
